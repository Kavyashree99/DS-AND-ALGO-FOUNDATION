/**
*Java provides many built-in classes and interfaces to implement hashing easily.
  *That is, without creating any HashTable or hash function. 
  *Java mainly provides us with the following classes to implement Hashing:
...
HashMap is also similar to HashTables in Java but it is faster in comparison as it is not synchronized. 
HashMap is used to store key-value pairs or to map a given value to a given key. The general application
of HashMap is to count frequencies of elements present in an array or a list.
**/
import java.util.*;
...
/**HashSet (Similar to HashMap, but maintains only keys, not pair): 
The HashSet class implements the Set interface, backed by a hash table which is actually a HashMap instance.
The class also offers constant time performance for the basic operations like add, remove, contains, and size assuming that the hash function disperses the elements properly among the buckets. 
